...
class PatientTable extends LivewireTableComponent
{
    ...
    {
        return [
            Column::make(__('messages.patient.patient_unique_id'), 'patient_unique_id')->searchable(
                function (Builder $query, $direction) {
                    return Patient::whereRaw('patient_unique_id', 'like', "%{$direction}%");
                }
            ),
                ...
                ->searchable(
                    function (Builder $query, $direction) {
                        return $query->whereHas('user', function (Builder $q) use ($direction) {
                            $q->whereRaw("TRIM(CONCAT(first_name)) like '%{$direction}%'");
                        });
                    }
                ),
            Column::make(__('messages.doctor_dashboard.total_appointments'), 'id')->view('patients.components.total_appointments'),
